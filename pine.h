...
void sub_odd(uint8_t * data){
  for(int i = 1; i <= 15; i += 2 ){
    data[i] = s_box[data[i]];
  }
}

void rsub_even(uint8_t * data){
  for(int i = 0; i <= 14; i += 2 ){
    data[i] = rs_box[data[i]];
  }
}
void rsub_odd(uint8_t * data){
  for(int i = 1; i <= 15; i += 2 ){
    data[i] = rs_box[data[i]];
  }
}

void sweep_left(uint8_t * data){
  for(int i = 14; i >= 0; --i){
    data[i] ^= data[i + 1];
  }
  data[15] ^= data[0];
...
void sweep_rev_left(uint8_t * data){
  data[15] ^= data[0];
  for(int i = 0; i <= 14; ++i){
    data[i] ^= data[i + 1];
  }
...
void sweep_rev_right(uint8_t * data){
  data[0] ^= data[15];
  for(int i = 15; i >= 1; --i){
    data[i] ^= data[i - 1];
  }
...
void round_key_gen(uint8_t * key, uint8_t round_keys[ROUND_COUNT][16]){
